"""Test C5 with inside(a,β) constraint.

center(d,β) ∧ center(b,γ) ∧ on(e,β) ∧ on(e,γ) ∧
on(d,K) ∧ on(b,K) ∧ inside(a,β) ∧ ¬on(a,K) ∧ ¬ss(e,a,K) ∧ ¬on(e,K) ∧
...
for _ in range(3_000_000):
    ...
    # Arbitrary radii
    r_beta = random.uniform(0.5, 8)
    r_gamma = random.uniform(0.5, 8)
    if r_beta + r_gamma < dist_db or abs(r_beta - r_gamma) > dist_db:
        continue

    pts = circle_circle_intersections(dx, dy, r_beta, bx, by, r_gamma)
    if len(pts) < 2:
        continue

    # a must be inside β (distance from d less than r_beta)
    ax, ay = random.uniform(-5, 5), random.uniform(-5, 5)
    if math.hypot(ax-dx, ay-dy) >= r_beta:
        continue
    ...
    # M = line(a, b); d NOT on M
    if on_line(dx, dy, ax, ay, bx, by, tol=0.01):
        continue

    # Pick e opposite a relative to K
    e0, e1 = pts
...
if violations == 0:
    ...
    print(f"INVALID — {100*violations/tested:.1f}%")
