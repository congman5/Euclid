"""
Test if picking e on the SAME side of K as a gives the correct results.
"""
import math
import random
...
def test_same_side_e(n_tests=500000):
    """
    Full I.9: pick e on SAME side of K as a.
    """
    counts = {
    ...
    for _ in range(n_tests):
        ...
        if dist(d, f) < 0.3:
            continue
        if on_line(a, d, f) or on_line(d, a, f) or on_line(f, a, d):
            continue
    ...
    print(f"I.9 with e on SAME side of K as a — {valid} valid tests:")
    ...
    if total_ec > 0:
        print(f"\n  ss(e,c,M) TRUE: {100*counts['ss_ec_M']/total_ec:.1f}%")
    if total_eb > 0:
        print(f"  ss(e,b,N) TRUE: {100*counts['ss_eb_N']/total_eb:.1f}%")
